...
public class ChangesetManagerTest {
    ...
    private DatabaseContext createDatabaseContext() {
        ...
        credentials = new DatabaseLoginCredentials(DatabaseConstants.TASK_DEFAULT_HOST,
                ...
                DatabaseConstants.TASK_DEFAULT_PROFILE_SQL, DatabaseType.MYSQL);
    ...
    /**
     * Tests that the changeset manager allocates a single changeset for multiple entities owned by
     * a single user, a different changeset per user, and creates a new changeset for a user when
     * the maximum number of entities is reached.
     */
    @Test
    public void testChangesetAllocation() {
        ...
        for (int i = 0; i < 50000; i++) {
            ...
            if (i > 1) {
                Assert.assertEquals("User 1 changeset id should not be re-used.", changesetIdUser1Previous,
                        changesetIdUser1Current);
                Assert.assertEquals("User 2 changeset id should not be re-used.", changesetIdUser2Previous,
                        changesetIdUser2Current);
            }
        ...
        changesetIdUser1Current = manager.obtainChangesetId(user1);
        changesetIdUser2Current = manager.obtainChangesetId(user2);
        Assert.assertFalse("User 1 changeset id should change after the maximum number of entities has been reached.",
                changesetIdUser1Previous == changesetIdUser1Current);
        Assert.assertFalse("User 2 changeset id should change after the maximum number of entities has been reached.",
                changesetIdUser2Previous == changesetIdUser2Current);
