...
if [ ! $CMD -q --read-change-interval $WORKING_DIRECTORY --read-xml $OSM_FILE --apply-change --bbox left=$LEFT bottom=$BOTTOM right=$RIGHT top=$TOP --write-xml $TEMP_OSM_FILE ]; then
