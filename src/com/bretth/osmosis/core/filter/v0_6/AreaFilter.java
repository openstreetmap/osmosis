...
public abstract class AreaFilter implements SinkSource, EntityProcessor {
	...
	private void emitFilteredWay(Way way) {
		...
		filteredWay = new Way(way.getId(), way.getTimestamp(), way.getUser(), way.getVersion());
    ...
    private void emitFilteredRelation(Relation relation) {
		...
		filteredRelation = new Relation(relation.getId(), relation.getTimestamp(), relation.getUser(), relation.getVersion());
